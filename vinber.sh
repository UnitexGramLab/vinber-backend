...
# =============================================================================
# Copyright (C) 2015-2021 Université Paris-Est Marne-la-Vallée <unitex-devel@univ-mlv.fr>
#
...
# =============================================================================
UNITEX_BUILD_VINBER_VERSION="2.0.0"
...
# =============================================================================
UNITEX_BUILD_DEPS_FILENAME="${UNITEX_BUILD_DEPS_FILENAME:-$UNITEX_BUILD_VINBER_CODENAME_LOWERCASE.deps}"
...
UNITEX_VERSION_GRAMLAB_IDE_COMMIT_BRANCH="?"
UNITEX_VERSION_GRAMLAB_IDE_COMMIT_DATE="?"
UNITEX_VERSION_GRAMLAB_IDE_COMMIT_HASH="?"
UNITEX_VERSION_GRAMLAB_IDE_COMMIT_TAG="?"
}
# =============================================================================
...
# shellcheck disable=SC2034
{
UNITEX_VERSION_URL_REPOSITORY_CORE="https://github.com/UnitexGramLab/unitex-core"
...
function stage_unitex_doc_checkout() {
  ...
  git_clone_pull   "" \
                   "git://github.com/UnitexGramLab/unitex-doc-usermanual.git" \
                   "$UNITEX_BUILD_REPOSITORY_USERMANUAL_NAME"

  git_info    DOC_GIT_CLONE_DETAILS                 \
              "$UNITEX_BUILD_REPOSITORY_USERMANUAL_NAME"

  # shellcheck disable=SC2034
  UNITEX_VERSION_DOC_REVISION_NUMBER=$UNITEX_VERSION_REVISION_NUMBER
  #UNITEX_VERSION_DOC_REVISION_NUMBER=${DOC_GIT_CLONE_DETAILS[0]}

  # Saving SVN last date changed
  echo "${DOC_GIT_CLONE_DETAILS[2]}" > "$UNITEX_BUILD_TIMESTAMP_DIR/$UNITEX_BUILD_REPOSITORY_USERMANUAL_NAME.current"
...
function stage_unitex_doc_make() {
  ...
  if [ $UNITEX_BUILD_DOCS_UPDATE -ne 0 ]; then
    ...
    log_info "Updating"  "$UNITEX_BUILD_RELEASE_DIR/Src/$UNITEX_BUILD_REPOSITORY_USERMANUAL_NAME.txt"
    # shellcheck disable=SC2086
    git log --oneline --no-merges --first-parent --pretty=format:'%h - %d %s (%cr) <%an>' --abbrev-commit > "$UNITEX_BUILD_RELEASE_DIR/Src/$UNITEX_BUILD_REPOSITORY_USERMANUAL_NAME.txt"
    cp "$UNITEX_BUILD_RELEASE_DIR/Src/$UNITEX_BUILD_REPOSITORY_USERMANUAL_NAME.txt" "$UNITEX_BUILD_RELEASES_CHANGES_DIR/$UNITEX_BUILD_REPOSITORY_USERMANUAL_NAME.txt"
...
function stage_unitex_doc_dist() {
  ...
  if [ $UNITEX_BUILD_DOCS_DEPLOYMENT -eq 0 ]; then
    if [ $UNITEX_BUILD_DOCS_UPDATE     -ne 0 -a \
      ...
      cp "$UNITEX_BUILD_REPOSITORY_USERMANUAL_LOCAL_PATH/tex/UnitexManual_EN_utf8.pdf" "$UNITEX_BUILD_RELEASE_APP_MANUAL_DIR/en/UnitexManual.pdf"

      # copy UnitexManual_FR_utf8.pdf to App/manual/fr
      mkdir -p "$UNITEX_BUILD_RELEASE_APP_MANUAL_DIR/fr"
      cp "$UNITEX_BUILD_REPOSITORY_USERMANUAL_LOCAL_PATH/tex/UnitexManual_FR_utf8.pdf" "$UNITEX_BUILD_RELEASE_APP_MANUAL_DIR/fr/ManuelUnitex.pdf"

      # UnitexManual_EN_utf8.pdf to releases/man/Unitex-GramLab-3.1beta-usermanual-en.pdf
      log_info "Copying" "UnitexManual_EN_utf8.pdf to $UNITEX_BUILD_RELEASES_MAN_DIR/$UNITEX_PACKAGE_MAN_PREFIX-en.pdf"
      cp "$UNITEX_BUILD_REPOSITORY_USERMANUAL_LOCAL_PATH/tex/UnitexManual_EN_utf8.pdf" "$UNITEX_BUILD_RELEASES_MAN_DIR/$UNITEX_PACKAGE_MAN_PREFIX-en.pdf"
      ...
      cp "$UNITEX_BUILD_REPOSITORY_USERMANUAL_LOCAL_PATH/tex/UnitexManual_FR_utf8.pdf" "$UNITEX_BUILD_RELEASES_MAN_DIR/$UNITEX_PACKAGE_MAN_PREFIX-fr.pdf"
...
function stage_unitex_packaging_windows_checkout () {
  ...
  git_clone_pull   "" \
                   "git://github.com/UnitexGramLab/$UNITEX_BUILD_REPOSITORY_PACK_WIN_NAME.git" \
  ...
  echo "${PACKAGING_GIT_CLONE_DETAILS[2]}" > "$UNITEX_BUILD_TIMESTAMP_DIR/$UNITEX_BUILD_REPOSITORY_PACK_WIN_NAME.current"
...
function stage_unitex_packaging_windows() {
  ...
  if [ $UNITEX_BUILD_FREEZE_RELEASE -ne 1 ]; then
    ...
    check_for_updates UNITEX_BUILD_PACK_UPDATE "$UNITEX_BUILD_REPOSITORY_PACK_WIN_NAME"  \
  ...
  if [  $UNITEX_BUILD_ISSUES_BEFORE_PACKAGING_WINDOWS_MAKE -eq 0 ]; then
    ...
    # we update the log file
    log_info "Updating"  "$UNITEX_BUILD_RELEASE_DIR/Src/$UNITEX_BUILD_REPOSITORY_PACK_WIN_NAME.txt"
    push_directory "$UNITEX_BUILD_REPOSITORY_PACK_WIN_LOCAL_PATH"
    git log --oneline --no-merges --first-parent --pretty=format:'%h - %d %s (%cr) <%an>' --abbrev-commit > "$UNITEX_BUILD_RELEASE_DIR/Src/$UNITEX_BUILD_REPOSITORY_PACK_WIN_NAME.txt"
    pop_directory
    cp "$UNITEX_BUILD_RELEASE_DIR/Src/$UNITEX_BUILD_REPOSITORY_PACK_WIN_NAME.txt" "$UNITEX_BUILD_RELEASES_CHANGES_DIR/$UNITEX_BUILD_REPOSITORY_PACK_WIN_NAME.txt"
...
#   echo "$packaging_linux_x86_64_timestamp" > "$UNITEX_BUILD_TIMESTAMP_DIR/$UNITEX_BUILD_PACKAGE_LINUX_X86_64_SUFFIX.current"

#   # 2. Check for linux_x86_64 distribution package updates
#   check_for_updates UNITEX_BUILD_PACK_UPDATE "$UNITEX_BUILD_PACKAGE_LINUX_X86_64_SUFFIX"  \
#                     "$UNITEX_BUILD_DOCS_UPDATE"                   \
#                     "$UNITEX_BUILD_LING_UPDATE"                   \
#                     "$UNITEX_BUILD_GRAMLAB_IDE_UPDATE"            \
#                     "$UNITEX_BUILD_CORE_UPDATE"                   \
...
function stage_unitex_packaging_unix() {
  ...
  if [ $UNITEX_BUILD_FREEZE_RELEASE -ne 1 ]; then
    ...
    # 2. Packaging check for updates
    check_for_updates UNITEX_BUILD_PACK_UPDATE "$UNITEX_BUILD_REPOSITORY_PACK_UNIX_NAME"  \
                      "$UNITEX_BUILD_PACK_FORCE_UPDATE"             \
                      "$UNITEX_BUILD_DOCS_UPDATE"                   \
                      "$UNITEX_BUILD_LING_UPDATE"                   \
                      "$UNITEX_BUILD_GRAMLAB_IDE_UPDATE"            \
...
function stage_unitex_packaging_unix_checkout () {
  ...
  git_clone_pull   "" \
                   "git://github.com/UnitexGramLab/$UNITEX_BUILD_REPOSITORY_PACK_UNIX_NAME.git" \
  ...
  # Saving GIT last date changed
  echo "${PACKAGING_GIT_CLONE_DETAILS[2]}" > "$UNITEX_BUILD_TIMESTAMP_DIR/$UNITEX_BUILD_REPOSITORY_PACK_UNIX_NAME.current"
...
function stage_unitex_packaging_configure_installer_unix() {
  ...
  if [ $UNITEX_BUILD_PACK_UPDATE -ne 0 ]; then
    ...
    log_info "Conf installer" "Configuring Unix distributions"
    ...
    if [ $UNITEX_BUILD_PACK_HAS_ERRORS -ne 0 ]; then
      ...
      log_info "Conf finished" "Unix distribution configuration process completed successfully"
      # Release update lock
      $UNITEX_BUILD_TOOL_SED -i 's/UNITEX_BUILD_PACK_FORCE_UPDATE=2/UNITEX_BUILD_PACK_FORCE_UPDATE=0/' \
                                  "$UNITEX_BUILD_CONFIG_FILENAME"
    fi
...
function stage_unitex_packaging_unix_dist() {
  ...
  if [ $UNITEX_BUILD_PACK_DEPLOYMENT   -eq 0 ]; then
    if [ $UNITEX_BUILD_PACK_UPDATE     -ne 0 -a \
      ...
      # we update the log file
      log_info "Updating"  "$UNITEX_BUILD_RELEASE_DIR/Src/$UNITEX_BUILD_REPOSITORY_PACK_UNIX_NAME.txt"
      push_directory "$UNITEX_BUILD_REPOSITORY_PACK_UNIX_LOCAL_PATH"
      git log --oneline --no-merges --first-parent --pretty=format:'%h - %d %s (%cr) <%an>' --abbrev-commit > "$UNITEX_BUILD_RELEASE_DIR/Src/$UNITEX_BUILD_REPOSITORY_PACK_UNIX_NAME.txt"
      pop_directory
      cp "$UNITEX_BUILD_RELEASE_DIR/Src/$UNITEX_BUILD_REPOSITORY_PACK_UNIX_NAME.txt" "$UNITEX_BUILD_RELEASES_CHANGES_DIR/$UNITEX_BUILD_REPOSITORY_PACK_UNIX_NAME.txt"
...
function stage_unitex_lingua_check_for_updates() {
  ...
  # we update the global lingua log file
  log_info "Updating"  "$UNITEX_BUILD_RELEASES_CHANGES_DIR/$UNITEX_BUILD_REPOSITORY_LING_NAME.txt"
  # shellcheck disable=SC2086
  git log --oneline --no-merges --first-parent --pretty=format:'%h - %d %s (%cr) <%an>' --abbrev-commit > "$UNITEX_BUILD_RELEASE_DIR/Src/$UNITEX_BUILD_REPOSITORY_LING_NAME.txt"
...
function stage_unitex_lingua_dist() {
  ...
  if [ $UNITEX_BUILD_LING_DEPLOYMENT -eq 0 ]; then
    if [ $UNITEX_BUILD_LING_UPDATE     -ne 0 -a \
      ...
      push_directory "$UNITEX_BUILD_REPOSITORY_LING_LOCAL_PATH"
      export UNITEX_BUILD_RELEASE_DIR="$UNITEX_BUILD_RELEASE_DIR"
      exec_logged_command "install.lingua" "./install.sh" || {   \
          die_with_critical_error "Dist failed" "Error installing language resources"
      }
...
function stage_unitex_gramlab_ide_checkout () {
  ...
  git_clone_pull   "" \
                   "git://github.com/UnitexGramLab/$UNITEX_BUILD_REPOSITORY_GRAMLAB_IDE_NAME.git" \
                   "$UNITEX_BUILD_REPOSITORY_GRAMLAB_IDE_NAME"

  git_info    GRAMLAB_IDE_GIT_CLONE_DETAILS           \
              "$UNITEX_BUILD_REPOSITORY_GRAMLAB_IDE_NAME"

  # shellcheck disable=SC2034
  UNITEX_VERSION_GRAMLAB_IDE_REVISION_NUMBER=$UNITEX_VERSION_REVISION_NUMBER
  #UNITEX_VERSION_GRAMLAB_IDE_REVISION_NUMBER=${GRAMLAB_IDE_GIT_CLONE_DETAILS[0]}

  # Saving SVN last date changed
  echo "${GRAMLAB_IDE_GIT_CLONE_DETAILS[2]}" > "$UNITEX_BUILD_TIMESTAMP_DIR/$UNITEX_BUILD_REPOSITORY_GRAMLAB_IDE_NAME.current"
...
function stage_unitex_gramlab_ide_make() {
  ...
  UNITEX_BUILD_GRAMLAB_IDE_HAS_ERRORS=0
  if [ $UNITEX_BUILD_GRAMLAB_IDE_UPDATE     -ne 0 ]; then

    # GramLab configure
    configure_templated_files "$UNITEX_BUILD_REPOSITORY_GRAMLAB_IDE_LOCAL_PATH"

    # we update the log file
    log_info "Updating"  "$UNITEX_BUILD_RELEASES_CHANGES_DIR/$UNITEX_BUILD_REPOSITORY_GRAMLAB_IDE_NAME.txt"
    push_directory "$UNITEX_BUILD_REPOSITORY_GRAMLAB_IDE_LOCAL_PATH"
    # shellcheck disable=SC2086
    git log --oneline --no-merges --first-parent --pretty=format:'%h - %d %s (%cr) <%an>' --abbrev-commit > "$UNITEX_BUILD_RELEASE_DIR/Src/$UNITEX_BUILD_REPOSITORY_GRAMLAB_IDE_NAME.txt"
    pop_directory

    cp "$UNITEX_BUILD_RELEASE_DIR/Src/$UNITEX_BUILD_REPOSITORY_GRAMLAB_IDE_NAME.txt" "$UNITEX_BUILD_RELEASES_CHANGES_DIR/$UNITEX_BUILD_REPOSITORY_GRAMLAB_IDE_NAME.txt"

    push_directory "$UNITEX_BUILD_REPOSITORY_GRAMLAB_IDE_LOCAL_PATH"
    log_info "Compiling" "GramLab IDE sources"

    # we compile the sources
    exec_logged_command "ant.GramLab" "$UNITEX_BUILD_TOOL_ANT" || {
...
function stage_unitex_gramlab_ide_dist() {
  ...
  UNITEX_BUILD_GRAMLAB_IDE_DEPLOYMENT=$(( ! UNITEX_BUILD_GRAMLAB_IDE_HAS_ERRORS && ( UNITEX_BUILD_GRAMLAB_IDE_FORCE_DEPLOYMENT  ||  UNITEX_BUILD_FORCE_DEPLOYMENT ) ))
  if [ $UNITEX_BUILD_GRAMLAB_IDE_DEPLOYMENT -eq 0 ]; then
    if [ $UNITEX_BUILD_GRAMLAB_IDE_UPDATE     -ne 0 -a \
      ...
      push_directory "$UNITEX_BUILD_REPOSITORY_GRAMLAB_IDE_LOCAL_PATH"
      export UNITEX_BUILD_RELEASE_DIR="$UNITEX_BUILD_RELEASE_DIR"
      exec_logged_command "ant.install.GramLab" "$UNITEX_BUILD_TOOL_ANT" install
      pop_directory
      ...
      # Finally, we export the source files
      log_info "Copying" "IDE sources"
...
function stage_unitex_core_logs_checkout() {
  ...
  git_clone_pull   "" \
                   "git://github.com/UnitexGramLab/$UNITEX_BUILD_REPOSITORY_LOGS_NAME.git" \
                   "$UNITEX_BUILD_REPOSITORY_LOGS_NAME"

  git_info    LOGS_GIT_CLONE_DETAILS             \
              "$UNITEX_BUILD_REPOSITORY_LOGS_NAME"

  # shellcheck disable=SC2034
  UNITEX_VERSION_LOGS_REVISION_NUMBER=$UNITEX_VERSION_REVISION_NUMBER
  #UNITEX_VERSION_LOGS_REVISION_NUMBER=${LOGS_GIT_CLONE_DETAILS[0]}

  # Saving SVN last date changed
  echo "${LOGS_GIT_CLONE_DETAILS[2]}" > "$UNITEX_BUILD_TIMESTAMP_DIR/$UNITEX_BUILD_REPOSITORY_LOGS_NAME.current"
...
function stage_unitex_core_make() {
  ...
  if [ $UNITEX_BUILD_CORE_UPDATE -ne 0 ]; then
    ...
    # shellcheck disable=SC2034
    UNITEX_VERSION_CORE_REVISION_NUMBER=$UNITEX_VERSION_REVISION_NUMBER
    #UNITEX_VERSION_CORE_REVISION_NUMBER=${CORE_GIT_CLONE_DETAILS[0]}

    # To keep update Version.h
    push_directory "$UNITEX_BUILD_REPOSITORY_CORE_LOCAL_PATH"
    # shellcheck disable=SC2034
    {
    UNITEX_VERSION_COMMIT_BRANCH_CORE=$(git rev-parse --abbrev-ref HEAD)
    UNITEX_VERSION_COMMIT_DATE_CORE=${CORE_GIT_CLONE_DETAILS[2]}
    UNITEX_VERSION_COMMIT_HASH_CORE=$(git rev-parse HEAD)
    UNITEX_VERSION_COMMIT_TAG_CORE=$(git describe --tags --abbrev=0)
    }
    pop_directory
    ...
    # stage_unitex_core_update_source_revision_header "${CORE_GIT_CLONE_DETAILS[0]}"

    # we update the log file
    log_info "Updating"  "$UNITEX_BUILD_RELEASE_DIR/Src/$UNITEX_BUILD_REPOSITORY_CORE_NAME.txt"
    push_directory "$UNITEX_BUILD_REPOSITORY_CORE_NAME"
    git log --oneline --no-merges --first-parent --pretty=format:'%h - %d %s (%cr) <%an>' --abbrev-commit > "$UNITEX_BUILD_RELEASE_DIR/Src/$UNITEX_BUILD_REPOSITORY_CORE_NAME.txt"
    pop_directory
    cp "$UNITEX_BUILD_RELEASE_DIR/Src/$UNITEX_BUILD_REPOSITORY_CORE_NAME.txt" "$UNITEX_BUILD_RELEASES_CHANGES_DIR/$UNITEX_BUILD_REPOSITORY_CORE_NAME.txt"
...
function git_info() {
  ...
  git_info_last_author_name=$(git --no-pager show -s --format="%aN")
...
function git_clone_pull() {
  ...
  if [ $is_git_repository -ne 0 ]; then
    ...
    # Cloning out
    log_info  "Repository" "$git_clone_pull_url"
    exec_logged_command "git.clone" "$UNITEX_BUILD_TOOL_GIT" clone -b master \
...
function stage_unitex_core_checkout() {
  ...
  git_clone_pull   "" \
                   "git://github.com/UnitexGramLab/$UNITEX_BUILD_REPOSITORY_CORE_NAME.git" \
                   "$UNITEX_BUILD_REPOSITORY_CORE_NAME"


  git_info            CORE_GIT_CLONE_DETAILS           \
                    "$UNITEX_BUILD_REPOSITORY_CORE_NAME"
  ...
  # Saving GIT last date changed
  echo "${CORE_GIT_CLONE_DETAILS[2]}" > "$UNITEX_BUILD_TIMESTAMP_DIR/$UNITEX_BUILD_REPOSITORY_CORE_NAME.current"
...
function stage_unitex_core() {
  ...
  if [ $UNITEX_BUILD_FREEZE_RELEASE -ne 1 ]; then
    ...
    check_for_updates UNITEX_BUILD_CORE_UPDATE "$UNITEX_BUILD_REPOSITORY_CORE_NAME"  \
  ...
  if [ $UNITEX_BUILD_SKIP_ULP_TESTS  -ne 1 -a \
    ...
    if [ $UNITEX_BUILD_HAS_UNITEXTOOLLOGGER -ne 0 ]; then
      ...
      exec_logged_command "UnitexToolLogger.ulpfile"                                   \
                          "$UNITEX_BUILD_REPOSITORY_LOGS_LOCAL_PATH/unitex-core-test.sh" \
                          -w80                                                           \
                          -c0                                                            \
                          -p1 || {
      ...
      if [ $RUNLOG_EXECUTION_FAIL -ne 0 ]; then
        ...
        log_notice  "[TEST PASS]"  "RunLog does not detect any regression replaying tests"
      fi
      ...
      if [ $UNITEX_BUILD_LOGS_HAS_ERRORS         -eq 0 -a \
        ...
        exec_logged_command "UnitexToolLogger.ulpvalgrind"                                 \
                            "$UNITEX_BUILD_REPOSITORY_LOGS_LOCAL_PATH/unitex-core-test.sh" \
                            -w80                                                           \
                            -R0                                                            \
                            -c0                                                            \
        ...
        if [ $VALGRIND_EXECUTION_FAIL -ne 0 ]; then
          ...
          log_error "[TEST FAIL]" "RunLog with Valgrind finish with errors"
          UNITEX_BUILD_LOGS_HAS_ERRORS=1
        else
          log_notice  "[TEST PASS]"  "Valgrind does not detect any memory leaks replaying logs"
        fi
  ...
  if [ $UNITEX_BUILD_CORE_DEPLOYMENT  -eq 0 ]; then
   if [ $UNITEX_BUILD_CORE_UPDATE     -ne 0 -a \
      ...
      cat "$UNITEX_BUILD_REPOSITORY_CORE_LOCAL_PATH/LICENSE"         <(echo) \
          "$UNITEX_BUILD_REPOSITORY_GRAMLAB_IDE_LOCAL_PATH/LICENSE"  <(echo) \
...
function stage_unitex_deployment_check() {
  ...
  #UNITEX_BUILD_PROGRAMS_DEPLOYMENT=$(( 0 || UNITEX_BUILD_GRAMLAB_IDE_DEPLOYMENT || UNITEX_BUILD_CORE_DEPLOYMENT ))
  ...
  if [ $UNITEX_BUILD_PACK_UPDATE -ne 0 ]; then
    ...
    exec_logged_command "nsis.win.installer" "$UNITEX_BUILD_TOOL_MAKENSIS"               \
      ...
      $nsis_arch_64_parameter     -DOUTPUT_RELEASES_DIR="$UNITEX_BUILD_RELEASES_BASEDIR" \
                                  "$UNITEX_BUILD_REPOSITORY_PACK_WIN_LOCAL_PATH/unitex.nsi" || {
        UNITEX_BUILD_PACK_HAS_ERRORS=1
      }
...
__END__
# shellcheck disable=SC2945
file_size_human_readable "$UNITEX_BUILD_RELEASES_LINUX_I686_DIR/$UNITEX_PACKAGE_LINUX_I686_PREFIX.run" >> "$UNITEX_BUILD_DOWNLOAD_WEB_PAGE"
# shellcheck disable=SC2945
cat >> "$UNITEX_BUILD_DOWNLOAD_WEB_PAGE" <<__END__
)
...
__END__
# shellcheck disable=SC2945
file_size_human_readable "$UNITEX_BUILD_RELEASES_LINUX_X86_64_DIR/$UNITEX_PACKAGE_LINUX_X86_64_PREFIX.run" >> "$UNITEX_BUILD_DOWNLOAD_WEB_PAGE"
# shellcheck disable=SC2945
cat >> "$UNITEX_BUILD_DOWNLOAD_WEB_PAGE" <<__END__
)
...
__END__
# shellcheck disable=SC2945
file_size_human_readable "$UNITEX_BUILD_RELEASES_OSX_DIR/$UNITEX_PACKAGE_OSX_PREFIX.run" >> "$UNITEX_BUILD_DOWNLOAD_WEB_PAGE"
# shellcheck disable=SC2945
cat >> "$UNITEX_BUILD_DOWNLOAD_WEB_PAGE" <<__END__
)
...
This package contains GramLab IDE together with the Windows (32-bit, 64-bit), GNU/Linux (i686, x86_64) and OS X (10.7+) executables.
<br/>
<br/>
...
<a href="$UNITEX_BUILD_URL_WEBSITE/$UNITEX_BUILD_RELEASES_HOME_NAME/$UNITEX_BUILD_RELEASES_LATESTDIR_NAME/$UNITEX_BUILD_RELEASES_CHANGES_HOME_NAME/$UNITEX_BUILD_REPOSITORY_CORE_NAME.txt">Changes on Core sources</a>
...
function check_previous_issues() {
  ...
  UNITEX_BUILD_HAS_PREVIOUS_ISSUES=$(( UNITEX_BUILD_HAS_PREVIOUS_ISSUES || UNITEX_BUILD_DOCS_FORCE_UPDATE        > 1                            || UNITEX_BUILD_PACK_FORCE_UPDATE        > 1                            || UNITEX_BUILD_LING_FORCE_UPDATE        > 1                            || UNITEX_BUILD_GRAMLAB_IDE_FORCE_UPDATE > 1                            || UNITEX_BUILD_LOGS_FORCE_UPDATE        > 1                            || UNITEX_BUILD_CORE_FORCE_UPDATE        > 1 ))
...
function setup_release_information() {
  ...
  # preliminary test before continue
  wget --no-check-certificate -qO- $UNITEX_VERSION_URL_REPOSITORY_CORE &> /dev/null || {
    UNITEX_VERSION_RELEASE="$UNITEX_BUILD_RELEASE"
    die_with_critical_error "GIT error" "Unable to access $UNITEX_VERSION_URL_REPOSITORY_CORE"
  }
  ...
  UNITEX_BUILD_RELEASES_REVISION_FILE="$UNITEX_BUILD_RELEASES_VERSION_BASEDIR/.revision"

  # setup the version revision
  # e.g 3816
  # check if UNITEX_VERSION_REVISION_NUMBER environment variable is already set
  if [ -z "${UNITEX_VERSION_REVISION_NUMBER}" ]; then
    # check if incremental revision number file exists
    if [ -e "$UNITEX_BUILD_RELEASES_REVISION_FILE" ]; then
      UNITEX_VERSION_REVISION_NUMBER=$(<"$UNITEX_BUILD_RELEASES_REVISION_FILE")
    else
      UNITEX_VERSION_REVISION_NUMBER=-1
    fi
    # increment global revision number
    UNITEX_VERSION_REVISION_NUMBER=$((${UNITEX_VERSION_REVISION_NUMBER##*.}+1))
  fi
...
function setup_build_environment() {
  ...
  UNITEX_BUILD_REPOSITORY_CORE_NAME="unitex-core"
  ...
  UNITEX_BUILD_REPOSITORY_GRAMLAB_IDE_NAME="gramlab-ide"
  ...
  UNITEX_BUILD_REPOSITORY_USERMANUAL_NAME="unitex-doc-usermanual"
  ...
  UNITEX_BUILD_REPOSITORY_LING_NAME="unitex-lingua"
  ...
  UNITEX_BUILD_REPOSITORY_PACK_WIN_NAME="unitex-packaging-windows"
  ...
  UNITEX_BUILD_REPOSITORY_PACK_UNIX_NAME="unitex-packaging-unix"
  ...
  UNITEX_BUILD_REPOSITORY_LOGS_NAME="unitex-core-tests"
  ...
  UNITEX_BUILD_RELEASE_SRC_CORE_DIR="$UNITEX_BUILD_RELEASE_SRC_DIR/$UNITEX_BUILD_REPOSITORY_CORE_NAME"
...
function process_command_line() {
  ...
  if [[ $(stat --format %a "$UNITEX_BUILD_CONFIG_FILENAME") == 600 ]]; then
    ...
    echo "Configuration file $UNITEX_BUILD_CONFIG_FILENAME, should not be world writable!, try:"
    echo "chmod 600 $UNITEX_BUILD_CONFIG_FILENAME"
    exit 1
  fi
}  # function process_command_line()
# =============================================================================
